...
function renderList(f) {
  _hf = f || "all";
  const filtered = _hf === "all" ? HACKATHONS : HACKATHONS.filter((h) => h.status === _hf);
  return `
<div class="wrap page">
  ...
  <div class="ftabs">
    ...
    <button class="ftab${_hf === s ? " active" : ""}" onclick="go('/hackathons?status=${s}')">
      ${s === "all" ? "전체" : slabel(s)}
      <span style="opacity:.6">${s === "all" ? HACKATHONS.length : HACKATHONS.filter((h) => h.status === s).length}</span>
    </button>`).join("")}
  </div>
  <div class="grid">
    ${filtered.length
      ? filtered.map(hCard).join("")
      : `<div class="empty" style="grid-column:1/-1"><div class="ico">📭</div><p>해당 상태의 해커톤이 없습니다.</p></div>`}
  </div>
</div>`;
}
...
// ── INIT ────────────────────────────────────────────────────
if (document.getElementById("app")?.dataset.page === "hackathons") {
  const q = new URLSearchParams(window.location.search);
  document.getElementById("app").innerHTML = renderList(q.get("status") || "all");
